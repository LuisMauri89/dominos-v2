...
func (b *httpHandlerBuilder) initEndpoints(m *mux.Router, options []httptransport.ServerOption) {
	for _, epCfg := range b.endpoints {
		...
		ep = MakeDefaultEntryEndpoint(epCfg.Service, b.logger)(epCfg.Endpoint)
		...
		if b.metricsCfg != nil {
			ep = EndpointSuccesAndFailureMetricsMiddleware(b.metricsCfg.SuccessfulRequests, b.metricsCfg.FailedRequests)(ep)
			ep = EndpointTimeTakenMetricsMiddleware(epCfg.Service, b.metricsCfg.RequestDuration)(ep)
		}
...
func (b *httpHandlerBuilder) resolveTracingOptions(epCfg EndpointConfig, options []httptransport.ServerOption) []httptransport.ServerOption {
	if b.openTracer != nil {
		...
		beforRequestFunc := opentracing.HTTPToContext(b.openTracer, operationName, b.logger)
